import { Transaction } from '../api/main'
...
export interface ISigner<Tx extends Transaction, Signer> {
  signMessage(signer: Signer, msg: string): Promise<string>
  signAndSendTx(signer: Signer, tx: Tx): Promise<string>
}
...
export class SignerFactory {
  ...
  registerSigner<Tx extends Transaction, Signer>(
    txType: TransactionType,
    signer: ISigner<Tx, Signer>
  ): void {
    this.signers[txType] = signer
  }

  getSigner<Tx extends Transaction, Signer>(
    txType: TransactionType
  ): ISigner<Tx, Signer> {
    if (this.signers && txType in this.signers) return this.signers[txType]
    throw new WalletError(WalletErrorCode.NOT_IMPLEMENTED)
  }
}
